...
def contar_orgaos_do_distrito(distrito):
    ...
    with open("trabalgo/aquaviario.csv", "r") as distritos:
        ...
        for i in linhas[1:]: #para todo elemento na linha ignorando a primeiro, então a partir da segunda
            campos = i.strip().split(',') # remover os espaçoes vazios e colocar no lugar deles uma "," 
            if campos[0] == distrito: # se a coluna 0  é equivalente a distrito ele ira adicionar a aumentar de 1 em 1 a qnt de orgãos 
                contador += 1
...
def listar_orgaos_do_distrito(distrito):
    with open("trabalgo/aquaviario.csv", "r") as distritos:# ler o arquivo
